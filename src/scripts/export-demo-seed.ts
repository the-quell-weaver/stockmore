 ...
 * - Collisions are resolved greedily: the first item keeps its slug, each
 *   subsequent collision gets the lowest available "_N" suffix (N ≥ 2).
 *   The assigned-Set check ensures suffixed names never collide with a
 *   pre-existing non-suffixed slug (e.g. A, A, A_2 → a, a_2, a_2_2).
 */
...
async function main() {
  ...
  // Exclude batches belonging to soft-deleted items: those items are absent
  // from itemRefById, and emitting itemRef:"unknown" would make the fixture
  // fail seeding with a missing-ref error.
  const seedBatches = (batches ?? [])
    .filter((b) => itemRefById.has(b.item_id))
    .map((b) => ({
      itemRef: itemRefById.get(b.item_id)!,
