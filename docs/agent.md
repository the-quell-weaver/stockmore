...
- 依 `docs/testing_guide.md` 指引自行跑測試（在開發/修復期間至少跑相關範圍）。
- 需要修復測試時，必須遵循「跑測試 → 看結果 → 修正 → 再跑」的迴圈，直到測試通過。
- 每次變更必跑完整測試清單：`test:unit`、`test:integration`、`test:e2e`。
- 若環境限制導致無法執行 `test:e2e`，必須明確說明原因與替代執行方式（不得省略）。

## 6.3 E2E 執行方式（Codex CLI 必用）

- Codex CLI 模式具有執行 E2E 的權限；在此模式應使用 `scripts/testing/run-e2e.sh` 執行。
- 請勿改用手動步驟；統一透過腳本確保環境準備、服務啟動與測試一致。

## 6.1 文件更新規則（避免誤刪）

- **只做最小增量變更**：在既有模板內補填，不重寫全文。
- **禁止移除模板段落**：除非使用者明確要求精簡或刪除。
- **若 diff 出現刪除**：必須先詢問使用者是否接受再繼續。
